...
def set_dacs(voltage1, voltage2):
    ...
    if code1 is not None:
        ...
        bin_str1 = format(code1,'012b')
        print(f"MACD 12-bit binary value to DAC1:{bin_str1}")
    else:
        print(f"Voltage1 {v1}V out of range")
        
    if code2 is not None:
        write_dac(DAC2_ADDR, code2)
        bin_str2 = format(code2,'012b')
        print(f"EMA9 12-bit binary value to DAC2:{bin_str2}")
    else:
...
try:
    while True:
        vin1 = float(input("Enter value for MACD (0–5): "))
        vin2 = float(input("Enter value for EMA9 (0–5): "))
        set_dacs(vin1, vin2)
        time.sleep(0.5)
